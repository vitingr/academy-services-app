...
const Service = ({ content, handleFunction }: ServicePageProps) => {
  ...
  const removeService = async () => {
    try {
      let servicesSubscribers = (content.subscribers).split(" ")
      let indice = servicesSubscribers.indexOf(data._id)
      servicesSubscribers.splice(indice, 1)
      let newServiceSubscribers = servicesSubscribers.join(" ")
      ...
      const response = await fetch("http://localhost:3030/services/update", {
        ...
        body: JSON.stringify({
          ...
          Description: content.description,
          Price: Number(content.price),
          Subscribers: newServiceSubscribers,
          Title: content.title,
        })
      })
      if (response.ok) {
  ...
  return (
    <div className='p-6 border border-neutral-200 rounded-lg w-[300px]'>
      ...
      {content.subscribers.includes(data._id) ? (
        <button className='text-[#F2921D] mt-10 p-2 w-full text-center' onClick={() => removeService()}>
          Cancelar Serviço
        </button>
      ) : (
